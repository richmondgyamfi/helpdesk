<nav class="navbar navbar-primary fixed-top bg-dark flex-md-nowrap p-0 shadow">
  <button class="navbar-toggler" type="button" data-toggle="collapse" data-target="#navcol">
        <span class="navbar-toggler-icon"></span>
      </button>
      <a class="navbar-brand col-sm-3 col-md-2 mr-0" href="#">Help System</a>
      <ul class="navbar-nav px-3">
        <li class="nav-item text-nowrap">
          <!--<a class="nav-link " href="#logout"><button class="btn btn-">Sign out</button></a>-->
        </li>
      </ul>
    </nav>

    <div class="container-fluid collapse navbar-collapse" id="navcol">
      <div class="row">
        <nav class="col-md-2 d-none d-md-block bg-light sidebar">
          <div class="sidebar-sticky">
            <ul class="nav flex-column">
              <br/>
              <p>
              <li class="nav-item" style="margin-bottom: 10px;">
                <a class="btn btn-info nav-link active" href="#">
                  <!-- <span class="fa fa-home"></span> -->
                  Dashboard <span class="sr-only">(current)</span>
                </a>
              </li>
             <!--  <li class="nav-item" style="margin-bottom: 10px;">
                <a class="nav-link btn btn-primary" href="employee_list.php">
                  <span class="fa fa-file"></span> 
                  Employee List
                </a>
              </li>
              <li class="nav-item" style="margin-bottom: 10px;">
                <a class="nav-link btn btn-primary" href="leave_due_list.php">
                  <span class="fa fa-setting"></span>
                  Leave Due List
                </a>
              </li>
              <li class="nav-item" style="margin-bottom: 10px;">
                <a class="nav-link btn btn-secondary" href="leave_setup.php">
                  <span class="fa fa-users"></span>
                  Leave Setup 
                </a>
              </li>
              <li class="nav-item" style="margin-bottom: 10px;">
                <a class="nav-link btn btn-secondary" href="approve_leave.php">
                  <span data-feather="bar-chart-2"></span>
                  Approve Leave
                </a>
              </li>
              <li class="nav-item" style="margin-bottom: 10px;">
                <a class="nav-link btn btn-success" href="department.php">
                  <span data-feather="layers"></span>
                  Department
                </a>
              </li>-->
              <li class="nav-item">
                <a class="nav-link btn btn-danger" href="helpdesk.php">
                  <!-- <span data-feather="layers"></span> -->
                  Help Desk
                </a>
              </li>
              <br/>
              <p></p>
              <li class="nav-item">
                <a class="nav-link btn btn-waring" href="#">
                  <span data-feather="layers"></span>
                  Sign Out
                </a>
              </li>
            </ul>
          </div>
        </nav>
